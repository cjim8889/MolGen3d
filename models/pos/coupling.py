from survae.transforms.bijections import Bijection, ConditionalBijection
...
class ConditionalMaskedAffineCouplingFlow(ConditionalBijection):
    def __init__(self, 
        ar_net, 
        mask, 
        scaling_func=lambda x: torch.exp(x), 
        ...
        super(ConditionalMaskedAffineCouplingFlow, self).__init__()
    ...
    def forward(self, x, context, mask=None, logs=None):
        return self._transform(x, context, mask=mask, forward=True, logs=logs)

    def inverse(self, z, context, mask=None, logs=None):
        return self._transform(z, context, mask=mask, forward=False)

    
    def _transform(self, z, context, mask=None, forward=True, logs=None):
        z_masked = z * self.coupling_mask
        alpha, shift = self.ar_net(z_masked, context).chunk(2, dim=self.split_dim)
